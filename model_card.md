...
An exception to this was function 4, where the exploitation phase was followed by another exploration phase, returning to exploitation for the final query.
...
| 2 | Sample farthest point from observed data points and corners, BO with GP surrogate model and UCB acquisition function to assess candidates from grid search | Find centroid of points in promising band, RF ensemble model with UCB acquisition function to assess candidates from grid search, BO with global GP surrogate model and UCB or EI acquisition functions to assess candidates from random search within regions defined by decision tree model. |
...
__Key:__ _BO_: Bayesian Optimisation, _EI_: Expected Improvement, _GP_: Gaussian Process, _NN_: Neural Network, _PI_: Probability of Improvement, _RF_: Random Forests, _SVM_: Support Vector Machines, _UCB_: Upper Confidence Bound
